...
const state = {
  currentUser: null,
  users: [
    { id: "u_demo_1", name: "Aarav Sharma", email: "demo@roadguard.app", phone: "9876543210", password: "password123", role: "user" },
    { id: "u_admin_1", name: "RoadGuard Admin", email: "admin@roadguard.app", phone: "9999999999", password: "admin123", role: "admin" },
  ],
  roadIssues: [
...
function renderNavbar() {
  ...
  if (state.currentUser) {
    ...
    const adminLink = state.currentUser.role === "admin" ? `<a href="#/admin" class="btn-secondary btn-sm">Admin Dashboard</a>` : "";
    actions.innerHTML = `
      ${adminLink}
...
function renderLanding() {
  ...
  app(`
    ...
    <section class="container section">
      <h2>How it works</h2>
      <p class="muted" style="margin-bottom:24px;">From spotting a hazard to a safer commute, in four steps.</p>
      <div class="tab-bar" id="stepTabs">
        ${steps.map((s, i) => `<button type="button" class="tab-btn ${i === landingActiveStep ? "active" : ""}" data-step="${i}"><span class="step-num">0${i + 1}</span> ${s[0]}</button>`).join("")}
      </div>
      <div class="card tab-panel" id="stepPanel">
        <div class="feat-icon" style="margin-bottom:16px;">${steps[landingActiveStep][2]}</div>
        <h3 style="margin:0 0 8px;">${steps[landingActiveStep][0]}</h3>
        <p class="muted" style="margin:0;">${steps[landingActiveStep][1]}</p>
      </div>
    </section>
  ...
  document.querySelectorAll("#stepTabs .tab-btn").forEach((btn) => {
    btn.onclick = () => {
      landingActiveStep = Number(btn.dataset.step);
      renderLanding();
    };
  });
}
...
function renderLogin() {
  app(`
    <div class="auth-wrap">
      ...
      <p class="demo-hint">Demo user: demo@roadguard.app / password123<br>Demo admin: admin@roadguard.app / admin123</p>
    </div>
  `);
...
function renderRegister() {
  ...
  document.getElementById("regForm").addEventListener("submit", (e) => {
    ...
    const newUser = { id: "u_" + Date.now(), name, email, phone, password, role: "user" };
...
/* =========================================================
   ADMIN DASHBOARD — all complaints from all users
   ========================================================= */
function renderAdmin() {
  ...
  const all = [...state.complaints].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  if (all.length === 0) {
    app(`
      <div class="container-sm empty-state">
        ...
        <h3>No complaints yet</h3>
        <p>Complaints submitted by any user will show up here.</p>
      </div>
    `);
    return;
  }
  ...
  app(`
    <div class="container-sm" style="padding-top:24px; padding-bottom:50px;">
      ...
      <p class="muted" style="margin-top:-8px; margin-bottom:20px; font-size:.85rem;">All complaints from all users (${all.length} total)</p>
      ${all.map((c) => {
        const submitter = state.users.find((u) => u.id === c.userId);
        return `
        <div class="card complaint-item">
          <div class="complaint-head">
            <div style="display:flex; gap:14px;">
              ...
              <div>
                ...
                <h3 style="margin:2px 0;">${escapeHtml(c.issueType)}</h3>
                <p class="muted" style="font-size:.85rem; margin:0;">${escapeHtml(c.address)}</p>
                <p class="muted" style="font-size:.75rem; margin:4px 0 0;">Reported ${formatDate(c.createdAt)} by <strong>${escapeHtml(submitter ? submitter.name : "Unknown user")}</strong></p>
              </div>
            </div>
            <span class="badge badge-${c.severity}">${c.severity} severity</span>
          </div>
          ...
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
            <label class="label" style="margin:0; font-size:.75rem;">Update status:</label>
            <select class="input" style="width:auto; padding:6px 10px; font-size:.8rem;" data-complaint="${c.id}">
              ${TIMELINE_STEPS.map((s) => `<option value="${s}" ${s === c.status ? "selected" : ""}>${s}</option>`).join("")}
            </select>
          </div>
          ${renderTimeline(c.status)}
        </div>`;
  ...
  document.querySelectorAll("select[data-complaint]").forEach((sel) => {
    sel.onchange = () => {
      const complaint = state.complaints.find((c) => c.id === sel.dataset.complaint);
      if (complaint) {
        complaint.status = sel.value;
        showToast(`Updated ${complaint.complaintId} to "${sel.value}"`);
        renderAdmin();
      }
    };
  });
...
/* =========================================================
   MAP PAGE — real Leaflet map with hazard points + safest-route
   directions.

   Routing uses the free public OSRM demo API (no backend of our
   own — just a fetch() call), which returns real road-following
   routes snapped to the actual street network, so the line drawn
   on the map always follows roads and never cuts through blocks
   or buildings. When alternative routes are available, we score
   each one against the known hazard reports and draw whichever
   alternative comes closest to avoiding them.
   ========================================================= */
let rgMap = null; // Leaflet map instance — recreated each visit to /map

// Distance in meters from a point to a lat/lng segment, using a local
// flat-earth projection — accurate enough at city scale for hazard checks.
...
const HAZARD_PROXIMITY_M = 130; // how close a route has to pass to "count" as risky
const SEVERITY_WEIGHT = { High: 3, Medium: 2, Low: 1 };

// coords: array of [lng,lat] from an OSRM route geometry
function scoreRouteAgainstHazards(coords, hazards) {
  ...
  hazards.forEach((h) => {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
      const d = pointToSegmentMeters(h.lat, h.lng, coords[i][1], coords[i][0], coords[i + 1][1], coords[i + 1][0]);
      if (d < minDist) minDist = d;
      if (minDist < HAZARD_PROXIMITY_M) break;
    }
    if (minDist <= HAZARD_PROXIMITY_M) {
      weighted += SEVERITY_WEIGHT[h.severity] || 1;
      nearby.push(h);
    }
  });
  return { weighted, nearby };
}

function rgPinIcon(kind) {
  const cls = kind === "start" ? "start" : "dest";
  const emoji = kind === "start" ? "🧭" : "🏁";
  return L.divIcon({
    className: "",
    html: `<div class="rg-pin ${cls}"><span>${emoji}</span></div>`,
    iconSize: [30, 30],
    iconAnchor: [15, 28],
    popupAnchor: [0, -26],
  });
...
function renderMap() {
  const mapPageState = { query: "", start: null, destination: null, routing: false };

  app(`
    <div class="map-page-wrap">
      <div id="rgLeafletMap" class="leaflet-map-container"></div>
      <div class="map-search">
        <input id="mapSearchInput" placeholder="Search roads, places, landmarks…" />
      </div>
      ...
      <div class="map-btn-stack">
        <button class="map-locate-btn" id="resetRouteBtn" title="Clear route">↺</button>
        <button class="map-locate-btn" id="locateBtn" title="Locate me">📍</button>
      </div>
      <div class="card issue-card-float route-info-card" id="routeInfoCard" style="display:none;"></div>
    </div>
  `);

  if (rgMap) { rgMap.remove(); rgMap = null; }
  rgMap = L.map("rgLeafletMap", { zoomControl: true }).setView([19.09, 72.90], 11);
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 19,
    attribution: '&copy; <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a> contributors',
  }).addTo(rgMap);

  const hazardLayer = L.layerGroup().addTo(rgMap);
  const routeLayer = L.layerGroup().addTo(rgMap);
  let startMarker = null, destMarker = null;

  function paintHazards() {
    hazardLayer.clearLayers();
    const q = mapPageState.query.trim().toLowerCase();
    const filtered = q ? state.roadIssues.filter((i) => i.roadName.toLowerCase().includes(q)) : state.roadIssues;
    filtered.forEach((issue) => {
      const marker = L.circleMarker([issue.lat, issue.lng], {
        radius: 9,
        color: "#fff",
        weight: 2,
        fillColor: ISSUE_COLORS[issue.type] || "#2F6FED",
        fillOpacity: 0.95,
      }).addTo(hazardLayer);
      marker.bindPopup(`
        <div class="rg-popup">
          <h4>${escapeHtml(issue.roadName)}</h4>
          <p class="muted" style="margin:0; font-size:.78rem;">${escapeHtml(issue.type)} · ${issue.reportCount} reports</p>
          <span class="badge badge-${issue.severity}">${issue.severity} severity</span>
          <p class="muted" style="margin:6px 0 0; font-size:.75rem;">Status: ${escapeHtml(issue.status)}</p>
        </div>
      `);
      marker.on("click", (ev) => L.DomEvent.stopPropagation(ev));
    });
    if (filtered.length && !mapPageState.start) {
      try { rgMap.fitBounds(L.latLngBounds(filtered.map((i) => [i.lat, i.lng])), { padding: [40, 40], maxZoom: 13 }); } catch (e) { /* ignore */ }
    }
  }
  paintHazards();

  function updateHint() {
    const el = document.getElementById("routeHint");
    if (mapPageState.routing) {
      el.textContent = "Finding the safest road route…";
      el.style.display = "block";
    } else if (!mapPageState.start) {
      el.textContent = "Tap 📍 to use your location, or click the map to set your starting point.";
      el.style.display = "block";
    } else if (!mapPageState.destination) {
      el.textContent = "Now click anywhere on the map to set your destination.";
      el.style.display = "block";
    } else {
  ...
  function setStart(loc) {
    mapPageState.start = loc;
    if (startMarker) rgMap.removeLayer(startMarker);
    startMarker = L.marker([loc.lat, loc.lng], { icon: rgPinIcon("start") }).addTo(rgMap).bindPopup("Start");
    startMarker.on("click", (ev) => L.DomEvent.stopPropagation(ev));
    updateHint();
  }

  function setDestination(loc) {
    mapPageState.destination = loc;
    if (destMarker) rgMap.removeLayer(destMarker);
    destMarker = L.marker([loc.lat, loc.lng], { icon: rgPinIcon("dest") }).addTo(rgMap).bindPopup("Destination");
    destMarker.on("click", (ev) => L.DomEvent.stopPropagation(ev));
    updateHint();
    computeRoute();
  }
  ...
  rgMap.on("click", (e) => {
    ...
    if (!mapPageState.start) setStart(loc);
  ...
  document.getElementById("locateBtn").onclick = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setStart(loc);
        rgMap.setView([loc.lat, loc.lng], 14);
        showToast("Location found — now click the map to set your destination");
      },
  ...
  document.getElementById("resetRouteBtn").onclick = () => {
    mapPageState.start = null; mapPageState.destination = null; mapPageState.routing = false;
    if (startMarker) { rgMap.removeLayer(startMarker); startMarker = null; }
    if (destMarker) { rgMap.removeLayer(destMarker); destMarker = null; }
  ...
  document.getElementById("mapSearchInput").oninput = (e) => {
    mapPageState.query = e.target.value;
    paintHazards();
  };

  async function computeRoute() {
    if (!mapPageState.start || !mapPageState.destination) return;
    mapPageState.routing = true;
    updateHint();
    routeLayer.clearLayers();
    document.getElementById("routeInfoCard").style.display = "none";

    try {
      const { start, destination } = mapPageState;
      const url = `https://router.project-osrm.org/route/v1/driving/${start.lng},${start.lat};${destination.lng},${destination.lat}?alternatives=true&overview=full&geometries=geojson&steps=false`;
      const res = await fetch(url);
      const data = await res.json();
      if (data.code !== "Ok" || !data.routes || !data.routes.length) throw new Error("No route found");

      // Only unresolved reports count as live hazards to route around.
      const hazards = state.roadIssues.filter((h) => h.type !== "Resolved");
      const scored = data.routes.map((r) => ({ route: r, ...scoreRouteAgainstHazards(r.geometry.coordinates, hazards) }));
      scored.sort((a, b) => a.weighted - b.weighted || a.route.distance - b.route.distance);
      const best = scored[0];

      // Draw the routes that were NOT chosen first, muted, so the safest pick stands out.
      scored.slice(1).forEach((s) => {
        L.polyline(s.route.geometry.coordinates.map((c) => [c[1], c[0]]), {
          color: "#8E9BB3", weight: 3, opacity: 0.55, dashArray: "6,7",
        }).addTo(routeLayer);
      });

      const routeColor = best.weighted === 0 ? "#17B890" : best.weighted >= 3 ? "#E13B3B" : "#F2A93B";
      const routeLine = L.polyline(best.route.geometry.coordinates.map((c) => [c[1], c[0]]), {
        color: routeColor, weight: 6, opacity: 0.9, lineCap: "round", lineJoin: "round",
      }).addTo(routeLayer);
      rgMap.fitBounds(routeLine.getBounds(), { padding: [50, 50] });

      const km = (best.route.distance / 1000).toFixed(1);
      const mins = Math.max(1, Math.round(best.route.duration / 60));
      const verdictBadge = best.weighted === 0
        ? `<span class="badge badge-Low">✅ Safest route — avoids all known hazards</span>`
        : best.weighted >= 3
          ? `<span class="badge badge-High">⚠️ Lowest-risk route — still passes a high-severity hazard</span>`
          : `<span class="badge badge-Medium">⚠️ Lowest-risk route — passes near ${best.nearby.length} hazard${best.nearby.length > 1 ? "s" : ""}</span>`;

      const hazardListHtml = best.nearby.length
        ? `<ul class="route-hazard-list">${best.nearby.map((h) => `<li>${escapeHtml(h.roadName)} — ${escapeHtml(h.type)} (${h.severity})</li>`).join("")}</ul>`
        : "";
      ...
      card.innerHTML = `
        <div style="display:flex; justify-content:space-between; gap:8px;">
          ...
          <button id="closeRouteCard" style="background:none;border:none;cursor:pointer;font-size:1rem;">✕</button>
        </div>
        <p class="route-stats">${km} km · ~${mins} min · ${scored.length > 1 ? `${scored.length} routes compared` : "1 route found"}</p>
        ${verdictBadge}
        ${hazardListHtml}
      `;
      card.style.display = "block";
      document.getElementById("mapLegend").style.display = "none";
      document.getElementById("closeRouteCard").onclick = () => {
        card.style.display = "none";
        document.getElementById("mapLegend").style.display = "block";
      };
    } catch (err) {
      showToast("Couldn't calculate a route. Check your connection and try again.", "error");
    } finally {
      mapPageState.routing = false;
      updateHint();
    }
  }
}
